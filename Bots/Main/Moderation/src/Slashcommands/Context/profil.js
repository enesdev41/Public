...
module.exports = {
  ...
  async execute(interaction, client) {
	...
	.addComponents(
		...
		let nameData = await isimler.findOne({ guildID: allah.GuildID, userID: üye.id });
		let registerData = await register.findOne({ guildID: allah.GuildID, userID: üye.id });
	  
				 const roles = client.guilds.cache.get(allah.GuildID).members.cache.get(interaction.targetId).roles.cache.filter(role => role.id !== allah.GuildID).sort((a, b) => b.position - a.position).map(role => `<@&${role.id}>`);
				  const rolleri = []
				  if (roles.length > 6) {
	  ...
	  \` • \` Katılım Tarihi: <t:${Math.floor(client.guilds.cache.get(allah.GuildID).members.cache.get(interaction.targetId).joinedAt / 1000)}:R>
	  ...
	  `);
		if (client.guilds.cache.get(allah.GuildID).members.cache.get(interaction.targetId).permissions.has("ADMINISTRATOR") || Ayarlar.teyitciRolleri.some(x => client.guilds.cache.get(allah.GuildID).members.cache.get(interaction.targetId).roles.cache.has(x))) 
