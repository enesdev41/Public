...
module.exports = {
  conf: {
    aliases: ["underworld","ban"],
    name: "underworld",
    help: "underworld <Ramal/ID> <Süre> <Sebep>",
  ...
  run: async (client, message, args, embed) => {
    ...
    .addComponents(
        new MessageSelectMenu()
            .setCustomId('doom')
            .setPlaceholder(`${yazı}'n Underworld Atılma Süresi?`)
            .addOptions([
              { label: '1 Ay Underworld', value: 'doom1', emoji: {id: "997835429447479296"}},
                { label: '3 Ay Underworld', value: 'doom2', emoji: {id: "997835429447479296"}},
                { label: '6 Ay Underworld', value: 'doom3', emoji: {id: "997820375389962260"}},
                { label: '9 Ay Underworld', value: 'doom4', emoji: {id: "997820377076076564"}},
                { label: `İşlem İptal`, value: 'doom5', emoji: {id: "909485171240218634"}},
             ]),
    );
...
if (duration) {
  ...
  message.react(green) 
  const penal = await client.penalize(message.guild.id, member.user.id, "Doom", true, message.author.id, reason, true, Date.now() + duration);
  if(msg) msg.delete();
  await message.channel.send({ content:`${green} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle Underworld'a atıldı! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
  if (allah.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar Underworld'a atıldınız.`}).catch(() => {});
  ...
  var msg = await message.channel.send({ content: `${member.toString()} isimli kullanıcıyı Underworld gönderme sebebinizi menüden seçiniz.`, components: [row]})
}
...
if(interaction.values[0] === "doom1") {
await interaction.deferUpdate();
const duration = "4w" ? ms("4w") : undefined;
const reason = "1 Aylık Underworld Cezası";
...
const penal = await client.penalize(message.guild.id, member.user.id, "Doom", true, message.author.id, reason, true, Date.now() + duration);
if(msg) msg.delete();
interaction.followUp({ content:`${green} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle Underworld'a atıldı! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
if (allah.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar Underworld'a atıldınız.`}).catch(() => {});
...
if(interaction.values[0] === "doom2") {
await interaction.deferUpdate();
const duration = "12w" ? ms("12w") : undefined;
const reason = "3 Aylık Underworld Cezası";
...
if(msg) msg.delete();
interaction.followUp({ content:`${green} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle Underworld'a atıldı! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
if (allah.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar Underworld'a atıldıniz.`}).catch(() => {});
...
if(interaction.values[0] === "doom3") {
await interaction.deferUpdate();
const duration = "24w" ? ms("24w") : undefined;
const reason = "6 Aylık Underworld Cezası";
...
if(interaction.values[0] === "doom4") {
await interaction.deferUpdate();
const duration = "36w" ? ms("36w") : undefined;
const reason = "9 Aylık Underworld Cezası";
...
if(interaction.values[0] === "doom5") {
await interaction.deferUpdate();
if(msg) msg.delete();
interaction.followUp({ content: `${green} Underworld Atma işlemi başarıyla iptal edildi.`, ephemeral: true });
}
})
}
  },
