...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    await penals.find({ guildID: message.guild.id, userID: member.user.id, }).sort({ date: -1 }).exec(async (err, res) => {
        ...
        .addComponents(
          new MessageButton().setCustomId('dosya').setEmoji("🚫").setStyle('SECONDARY'),
          new MessageButton().setCustomId('sayi').setEmoji("❔").setStyle('SECONDARY'),
          new MessageButton().setCustomId('iptal').setEmoji("909485171240218634").setStyle('SECONDARY'),
        );
