...
module.exports = {
  conf: {
    aliases: ["ununderworld","af"],
    name: "ununderworld",
    help: "ununderworld <Ramal/ID>",
  ...
  run: async (client, message,  args, embed) => {
    ...
    return }
    if (!conf.doomRole.some(x => member.roles.cache.has(x))) 
    {
    message.react(red)
    message.channel.send({ content:"Bu üye underworld cezasında değil!"}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    if (!message.member.permissions.has(8n) && member.roles.highest.position >= message.member.roles.highest.position) 
    {
    message.react(red)
    message.channel.send({ content:"Kendinle aynı yetkide ya da daha yetkili olan birinin underworld cezasını kaldıramazsın!"}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    if (!member.manageable) {
    message.react(red)  
    message.channel.send({ content:"Bu üyeyi underworld cezasından çıkaramıyorum!"}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    ...
    const data = await penals.findOne({ userID: member.user.id, guildID: message.guild.id, $or: [{ type: "Doom" }, { type: "Doom" }], active: true });
    if (data) {
      data.active = false;
      await data.save();
    }
    message.react(green)
    message.reply({ content:`${green} ${member.toString()} üyesinin underworld cezası ${message.author} tarafından kaldırıldı!`}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    if (allah.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, underworldünü kaldırıldı!`}).catch(() => {});
