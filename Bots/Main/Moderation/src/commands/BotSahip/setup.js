...
setup2.forEach(async (x) => {
  ...
  let db = global.ozisetupxd.get(`${x.conf}`)
  if(rol) {
  if(db.some(ozi => ozi.includes(rol.id))) {
    global.ozisetupxd.pull(`${x.conf}`, `${rol.map(x => x)}`)
...
setup3.forEach(async (x) => {
  ...
  if (rol) {
    global.ozisetupxd.set(`${x.conf}`, `${rol.id}`)
...
setup4.forEach(async (x) => {
  ...
  if (channel) {
    global.ozisetupxd.set(`${x.conf}`, `${channel.id}`)
...
setup5.forEach(async (x) => {
  ...
  if(db.some(ozi => ozi.includes(kanal.id))) {
    global.ozisetupxd.pull(`${x.conf}`, `${kanal.map(x => x)}`)
...
setup6.forEach(async (x) => {
  ...
  global.ozisetupxd.set(`${x.conf}`, jaylen.id)
