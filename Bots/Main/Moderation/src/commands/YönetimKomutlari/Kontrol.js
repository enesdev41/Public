...
module.exports = {
    ...
    return }
       ...
       const etkinlik = await client.guilds.cache.get(allah.GuildID).roles.cache.find(x => x.name.includes(conf.etkinlik)).id
       const cekilis = await client.guilds.cache.get(allah.GuildID).roles.cache.find(x => x.name.includes(conf.cekilis)).id
...
.addComponents(
new MessageButton().setStyle('SECONDARY').setLabel('Etkinlik/Çekiliş Rol Dağıt').setCustomId('ecdagit'),
new MessageButton().setStyle('SECONDARY').setLabel('Tag Rol Dağıt').setCustomId('tagrol'),
new MessageButton().setStyle('SECONDARY').setLabel('Kayıtsız Rol Dağıt').setCustomId('kayıtsızdagit'),
);
