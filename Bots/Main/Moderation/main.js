  ...
  files.forEach(f => {
    fs.readdir("./src/commands/" + f, (err2, files2) => {
      files2.forEach(file => {
        ...
        console.log(`[Moderation Komut] ${props.conf.name} komutu yüklendi!`);
...
client
  ...
  fs.readdirSync('./src/Slashcommands/').forEach(async category => {
	...
	const rest = new REST({ version: '9' }).setToken(allah.Main.ModerationToken);
  (async () => {
	try {
		console.log('[Moderation] Slash ve Komutlar yükleniyor.');
		await rest.put(
			Routes.applicationGuildCommands(allah.Main.BotClientID, allah.GuildID),
			{ body: slashcommands },
		).then(() => {
			console.log('[Moderation] Slash ve Context Komutlar yüklendi.');
		});
	}
	catch (e) {
		console.error(e);
