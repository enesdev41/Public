...
module.exports = async (oldUser, newUser) => {
  ...
  const channel2 = client.channels.cache.find(x => x.name == "tagı_alan_log");
  const channel3 = client.channels.cache.find(x => x.name == "tagı_bırakan_log");
...
channel3.wsend({ content: `${member.toString()} [\` ${member.id} \`]`, embeds: [embed]});
...
\` ➥ \` Anlık taglı üye: **${guild.members.cache.filter(x => x.user.username.includes(conf.tag)).size}**
`);
channel2.wsend({ content: `${member.toString()} [\` ${member.id} \`]`, embeds: [embed]});
