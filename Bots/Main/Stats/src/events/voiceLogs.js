...
module.exports = async (oldState, newState) => {
  ...
  if (!oldState.channel && newState.channel) {
    ...
    channel.wsend({
      embeds: [new MessageEmbed().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true }) }).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 })).setColor("RANDOM")
        .setDescription(`
...
`)]
  ...
  if (oldState.channel && !newState.channel) {
    ...
    channel.wsend({
      embeds: [new MessageEmbed().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true }) }).setThumbnail(oldState.member.displayAvatarURL({ dynamic: true, size: 2048 })).setColor("RANDOM")
        .setDescription(`
...
`)]
  ...
  if (!oldState.serverMute && newState.serverMute) return channel3.wsend({
    embeds: [
      new MessageEmbed()
