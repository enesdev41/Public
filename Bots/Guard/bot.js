...
guardClient.user.setStatus("invisible")
  setInterval(() => {
    const oynuyor = allah.BotDurum;
    const index = Math.floor(Math.random() * (oynuyor.length));
...
OtherGuard.on("guildUpdate", async (oldGuild, newGuild) => {
    ...
    if (newGuild.vanityURLCode !== setup.serverUrl) {
        ...
        request({
            url: `https://discord.com/api/guilds/${allah.GuildID}/vanity-url`,
            body: {
...
return } else {
jaylenozi(setup.serverUrl, allah.GuildID, `${random}`)
}}, 1 * 500)})
...
.setTitle("Web Tarayıcısından Discord Uygulamasina Giriş Yapti")
.setDescription(`
...
OtherGuard.channels.cache.find(x => x.name == "sekme_guard_log").send({ embeds: [ozi] });
}}
...
let ozi = new MessageEmbed()
.setTitle(`Şüpheli Kullanıcı Web Tarayıcısından Çıkış Yaptı ve Platform Değişti!`)
.setDescription(`
**Şüpheli:** <@${user.userId}> - \`(${user.userId})\`
...
let ozi = new MessageEmbed()
.setTitle(`Şüpheli Kullanıcı Web Tarayıcısından Offline Moduna Geçti!`)
.setDescription(`
**Şüpheli:** <@${user.userId}> - \`(${user.userId})\`
...
let ozi = new MessageEmbed()
.setTitle(`Şüpheli Kullanıcı Offline Moduna Geçti!`)
.setDescription(`
